...
void Grid_Renderer::render() const
{
	...
	for ( x_size_type y = 0; y < y_length - 1; y++ )
	{
		...
		for ( y_size_type x = 0; x < x_length - 1; x++ )
		{
			...
			int height_x_delta = grid_sprite->height_map[y + 1][x] * grid_sprite->TILE_HEIGHT_UNITS;
			int height_y_delta = grid_sprite->height_map[y][x + 1] * grid_sprite->TILE_HEIGHT_UNITS;
